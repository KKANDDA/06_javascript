...
// 재할당이 금지된다.
// x = 2; // 근데 잡아주진 않는다. 돌려 봐야 알 수 있다.

// 일반적으로 상수의 이름은 대문자로 선언하고, 스네이크 케이스로 표현한다.
...
student.name = "유관순";
console.log(student); // 안의 내용물을 추가 변경 삭제가 가능하지만, 재할당은 안 된다.
student.height = 180;
console.log(student);
delete student.height;
console.log(student);
// student = {}; // 재할당은 안 된다. 프로퍼티 변경은 가능!

/*
전역 변수의 문제점..
    1. 모든 코드가 전역 변수를 참조하고 변경할 수 있는 암묵적 결합을 허용한다.
       변수의 유효 범위가 클수록 코드의 가독성은 나빠지고, 의도치 않게 변경 될 위험성도 높아진다.

    2. 전역 변수는 생명주기가 길다.
       메모리 리소스도 오래 소비하며, 상태 변경이 가능한 시간 및 기회가 많다.
       더욱이 중복된 변수명을 통해 의도치 않은 재할당이 일어날 수 있다.

    3. 스코프 체인 상에서 종점에 존재하기 때문에 검색 속도가 가장 느리다.
       => 변수의 스코프는 좁을 수록 좋다. ~~~~~~~~~~ 중요. 전역 변수의 스코프는 페이지 그 자체이다. 전역 변수를 가급적 쓰지 말자! 지역 변수 활용을 추천!
*/ 