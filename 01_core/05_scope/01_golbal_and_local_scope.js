...
전역은 변수를 선언하면 전역 스코프를 갖는 전역 변수가 되며 전역 변수는 어디서든지 참조할 수 있다.
...
function outer(){
    ...
    function inner(){
        ...
        console.log(x); // 변수의 은닉화.. 외부에 존재하는 동일한 변수를 덮었다.
        console.log(y);
        console.log(z);
    }
    inner();
...
// 자바, c 등 대부분의 프로그래밍 언어는 함수 몸체 뿐만 아니라 모든 코드 블록{if, for, while, try 등}
...
var i = 0;
for(var i=0; i<10; i++){}
console.log(i); // 바는 지역 스코프를 인정하지 않는다.

var i = 0;
for(let i=0; i<10; i++){}
console.log(i); // 랫은 바와 다르다. 자바와 비슷..